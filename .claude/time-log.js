window.TIME_LOG_DATA = [
  ...
  {
    "datetime": "2026-06-14T13:26:37Z",
    "phase": "1",
    "step": "10",
    "hours": 0.3,
    "who": "claude",
    "notes": "npm publish v0.1.0: removed @react-spring/web peerDep, rebuilt dist, Git Flow release branch, tag v0.1.0, back-merge to develop, GitHub Release created"
  },
  {
    "datetime": "2026-06-14T23:22:16Z",
    "phase": "2",
    "step": "0",
    "hours": 0.05,
    "who": "claude",
    "notes": "Opened Phase 2 — v0.2-features-and-polish"
  },
  {
    "datetime": "2026-06-14T23:27:30Z",
    "phase": "2",
    "step": "1",
    "hours": 0.4,
    "who": "claude",
    "notes": "Hotfix v0.1.1: ring-only glow (evenodd), two-pass draw, version bump, Git Flow hotfix branch, tag, npm publish, back-merge"
  },
  {
    "datetime": "2026-06-14T23:35:31Z",
    "phase": "2",
    "step": "2",
    "hours": 0.6,
    "who": "claude",
    "notes": "Phase 2 decomposition: gathered requirements, VoltAgent stack proposal, wrote PRD + EPIC + 8 task files for v0.2.0 (features, docs, CI, perf)"
  },
  {
    "datetime": "2026-06-15T00:00:12Z",
    "phase": "2",
    "step": "3",
    "hours": 0.255348315,
    "who": "claude",
    "notes": "Task 001 — sprite pre-render + batched draw loop: new src/sprites.ts (bounded-LRU sprite cache, drawGlowSprite ring-glow halo+core, getGlowSprite keyed by shape·color·size·glow·dpr, clearSpriteCache); refactored useParticleLoop.ts hot path to Map-lookup + drawImage (zero per-frame gradient alloc), dpr re-read + cache-invalidate on resize/zoom; 14 sprite tests added (30 total pass), tsc+eslint clean, build 8.35KB gzip. Code-review gate passed (2 rounds: B1 stale-dpr + B3 cache-bound fixed; minSize/maxSize clear kept per spec)."
  },
  {
    "datetime": "2026-06-15T00:24:26Z",
    "phase": "2",
    "step": "4",
  ...
  {
    "datetime": "2026-06-15T21:21:47Z",
    "phase": "2",
    "step": "10",
    "hours": 0.059605052500000005,
    "who": "claude",
    "notes": "Task 008 — release prep v0.2.0: README playground link + 'What's new in v0.2.0' section + star/twinkle/colorCycle example (props table already done in #16); bumped package.json 0.1.1→0.2.0; verified ≤12KB gzip budget (9.21KB ESM); confirmed JSDoc on all 15 props + fixed stale ctx.shadowBlur glowIntensity doc (rebuilt dist .d.ts). 58 tests pass, lint clean. Tag/publish is the separate user-gated release step."
  },
  {
    "datetime": "2026-06-15T22:03:13Z",
    "phase": "2",
    "step": "11",
    "hours": 0.10125506694444444,
    "who": "claude",
    "notes": "DPR-aware main canvas (retina blur fix): setSize backs canvas at clientW/H × devicePixelRatio + ctx.setTransform(dpr) so drawing stays in CSS coords; clearRect/updateParticle/spawn now use CSS dims; resize re-reads dpr before setSize (also fixes a latent stale-dpr resize bug). Sprites now blit 1:1 into the device-res backing → crisp cores, no browser upscale. Removed the DPR backlog item from SPLIT-PLAN §6. +1 regression test (canvas.width=cssW×dpr, setTransform(dpr) at dpr=2); 59 tests pass, lint clean, build 9.26KB gzip. Code-review: no blocking."
  },
  {
    "datetime": "2026-06-15T22:39:00Z",
    "phase": "2",
    "step": "12",
    "hours": 8.118333333333333e-06,
    "who": "claude",
    "notes": "Phase 2 release + close-out: cut release/0.2.0→main, tagged & published v0.2.0 (provenance fix #24 for repository.url); diagnosed+fixed hi-DPI blur (#26/#27, the deferred §6 retina item); v0.2.0 shipped blurry (run approved pre-fix) so superseded by v0.2.1 (#28) which also defaults glowIntensity→0; back-merged main→develop (#29); closed Phase 2 (closure log, SPLIT-PLAN §5 row, plan-data completed, render-check); self-healed close-phase.py python→sys.executable bug. cosmic-dust-canvas@0.2.1 live on npm + Pages playground."
  }
];
