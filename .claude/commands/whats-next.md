...
2. After the dashboard, ask **exactly one concise question**. The phrasing depends on the dashboard's NEXT ACTION — match the case below; never synthesize a different recommendation.
   ...
   | "Activate pre-commit locally…" | `Next: install pre-commit hooks (pip install pre-commit && pre-commit install). Want me to do that now? (y/n)` |
   ...
   | (otherwise) | `Next: open Phase 1 by adding a row to SPLIT-PLAN §5 (progress log). Want me to do that now? (y/n)` |
