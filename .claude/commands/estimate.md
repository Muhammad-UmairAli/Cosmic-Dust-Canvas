...
- Re-run for an existing step when scope changes — the script is idempotent (it updates `data-h` / `data-c` in place; it does NOT touch `data-actual` / `data-diff`, which are owned by `/log-time`).
...
```
/estimate 1 2 "Build SPA HTML + CSS in parallel (general-purpose + frontend-engineer)"
/estimate 2 3 "Integrate 11 theme CSS blocks; replace dark/light toggle with picker dropdown"
/estimate 3 1 "Parallel extract tokens from 59 themes across 3 batches of agents"
```

## What the command does

1. **Set the task context** so the agent log records the correct phase and step:
...
- Do not invent estimates yourself — always spawn the subagent. The kit's value here is *consistency*, and consistency requires every estimate to flow through the same rubric application.
