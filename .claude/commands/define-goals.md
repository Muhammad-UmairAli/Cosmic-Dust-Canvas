...
5. **Log the time.** This conversation is itself substantive work — capture it via `/log-time`:
   ```
   /log-time 0 16 <hours> "Defined SPLIT-PLAN §1 (goals) and SPLIT-PLAN §2 (out of scope) interactively"
   ```
   `<hours>` is your estimate of how long the back-and-forth took (typically 0.1–0.5h depending on how many goals were collected). Phase 0 step 16 is the kit's reserved slot for goal definition.
...
6. **Open Phase 1 and immediately hand off to `/work-the-phase`.**
   ...
   Derive a short Phase 1 title from the user's first goal: lowercase, hyphenated words, max 30 characters (e.g., "track-lacrosse-sticks"). Then open the phase by calling the script directly:
