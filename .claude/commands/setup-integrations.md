...
This wizard configures your project in 8 steps and only needs to run once, though you can re-run it anytime to change your answers:

- **1/8 Project name** — names your dashboard header
- **2/8 Branching model** — base flow (main only) or Git Flow (develop + release/* + hotfix/*)
- **3/8 Cloud provider** — UAT/PROD deployment target (Git Flow only)
- **4/8 CCPM** — parallel task decomposition with Git worktrees
- **5/8 VoltAgent** — 130+ specialist AI personas
- **6/8 Design identity** — pick multiple brand-inspired themes; sets dashboard default and instructs VoltAgent to build a theme picker in your app
...
Before asking any questions, run the following so agents spawned during this wizard are attributed to Phase 0 step 16 in the agent log:

```bash
python3 tools/start-task.py 0 16 "Integration wizard"
```
...
Skip cleanly on "no" / "n" / "skip"; never assume defaults.
...
Ask: **"Use Git Flow (develop, release/*, hotfix/*) or stick with the base flow (single `main`)? Git Flow is recommended for projects with deployable UAT/PROD environments. (gitflow / base / skip)"**
...
### 3/8  Cloud provider (only if Git Flow chosen)
...
### 4/8  CCPM (decomposition + parallel execution)
...
### 5/8  VoltAgent (specialist personas)

Ask: **"Install VoltAgent specialists (130+ persona library)? (y/n)"**

If yes, first attempt adding the marketplace source:
...
If proceeding (either Claude ran it or user confirmed they ran it), install all four plugin groups:
...
### 6/8  Design identity (multi-theme picker)

Ask: **"Set a visual design identity for this project? The kit ships 73 brand-inspired themes — you can select multiple dark and light themes. They'll set your dashboard default and instruct VoltAgent to build a theme picker in your app. (y/n)"**
...
### 7/8  Branch protection rulesets
...
If the user says no to creating a remote, skip branch protection (mark skipped) and continue to 7/7.
...
### 8/8  Local commit-time hooks (pre-commit)

Ask: **"Install pre-commit and activate hooks now? (y/n)"**

If yes: `pip install pre-commit` (or `pip3 install pre-commit`), then `pre-commit install`. Verify `.git/hooks/pre-commit` exists after.
...
```
/log-time 0 16 <hours> "Integration wizard"
```
...
- Don't auto-default questions the user didn't answer. If they say "skip" or are ambiguous, treat it as skip and move on.
- Don't run all seven actions in a batch — ask, wait, act, then move to the next question. The user might want to stop midway.
- Don't write the wizard marker unless the user actually walked through all the questions. If they abort mid-wizard, the wizard is incomplete and `/whats-next` should still show remaining steps.
