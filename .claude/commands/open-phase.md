...
Open Phase N formally. Every phase must be opened via this command **before** any phase work happens (writing PRDs, decomposing tasks, building, etc.). Skipping `/open-phase` and starting work directly is the bug that caused the lacrosse-stick-tracker session's CCPM decomposition to go untracked.
...
| `<short title>` | `"lacrosse-stick-tracker build"`, `"add billing UI"`, `"hotfix: timezone bug"` |
...
4. Calls `tools/log-time.py <phase> 0 0.05 "Phase <N> opened — ..."` which creates the Phase N block in DASHBOARD.html, adds the phase to the PHASES array, appends the TIME-LOG row, syncs, and verifies render-check.
