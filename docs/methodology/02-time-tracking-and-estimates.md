...
The Difference column has hover tooltips drawn from each step's Notes column in `TIME-LOG.md`, so the *why* is one click away.

## Estimating: the rubric

The Baseline and With-AI columns mean different things; they should carry different numbers. *Baseline* is what a senior dev typing every line, without an AI pair-programmer, would honestly take. *With AI* is the same dev with the AI helping. The point of having both is that the gap between them — visible on every step row, summed at the phase subtotal, totalled at the grand-total — is the kit's own running case study of where AI assistance pays off and where it doesn't.
