...
> The kit's `tools/setup-integrations.sh` wizard offers to download a `DESIGN.md` from getdesign.md and save it as `docs/DESIGN.md`. Read on for the full catalog, picking guidance, and how it composes with VoltAgent's design specialists.
...
| Layer | Mechanism |
|---|---|
| **VoltAgent** | *Agents* — invoked via the `Agent` tool from inside a task |
| **GetDesign** | *Context* — a markdown file dropped into the project so any agent reads it and follows it |
...
- **Specialist review still applies.** A UI task can still invoke VoltAgent's `accessibility-tester` or `ui-designer` for a focused review — GetDesign provides the *spec*, VoltAgent provides the *reviewer*.
