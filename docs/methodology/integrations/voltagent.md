...
> The kit's `tools/setup-integrations.sh` wizard installs a VoltAgent plugin (default: `voltagent-qa-sec`) via `claude plugin install`. Read on for the full catalog, recommended starter set, and when to invoke each specialist.
