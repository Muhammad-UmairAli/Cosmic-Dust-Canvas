...
if [[ -d .claude/ccpm ]]; then
  ...
  if command -v git >/dev/null 2>&1; then
    ...
    if git clone --quiet https://github.com/automazeio/ccpm .claude/ccpm 2>/dev/null; then
      done_msg "Cloned CCPM into .claude/ccpm."
      say "  Follow CCPM's own README for slash-command registration."
    else
      warn "git clone failed. Retry: git clone https://github.com/automazeio/ccpm .claude/ccpm"
    fi
  else
...
hdr "5/8  VoltAgent (specialist personas)"
say "  130+ specialists: code-reviewer, security-auditor, debugger,"
say "  language-pros, etc. See docs/methodology/integrations/voltagent.md."

if prompt_yn "Install VoltAgent specialists?" "y"; then
  if command -v claude >/dev/null 2>&1; then
    if claude plugin marketplace add VoltAgent/awesome-claude-code-subagents >/dev/null 2>&1; then
      done_msg "Marketplace added."
    else
      warn "Marketplace add failed (already added?)"
    fi
    PLUGIN=$(prompt_choice "Which VoltAgent plugin to install first?" "voltagent-qa-sec" \
      voltagent-qa-sec voltagent-core-dev voltagent-lang voltagent-meta skip)
    if [[ "$PLUGIN" != "skip" ]]; then
      if claude plugin install "$PLUGIN" >/dev/null 2>&1; then
        done_msg "Installed $PLUGIN."
      else
        warn "Install of $PLUGIN failed; run 'claude plugin install $PLUGIN' manually."
      fi
    fi
  else
    warn "Claude Code CLI ('claude') not on PATH. After installing it, run:"
    say "    claude plugin marketplace add VoltAgent/awesome-claude-code-subagents"
    say "    claude plugin install voltagent-qa-sec"
  fi
else
  skip_msg "VoltAgent — skipped."
fi
...
if prompt_yn "Install pre-commit and activate hooks now?" "y"; then
  ...
  if [[ -n "$PIP" ]]; then
    ...
    warn "pip / pip3 not on PATH. Install pre-commit manually:"
    say "    pip install pre-commit && pre-commit install"
  fi
else
  skip_msg "pre-commit — skipped. Run 'pip install pre-commit && pre-commit install' anytime."
fi
